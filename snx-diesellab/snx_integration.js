...
try {
await supabase.from(‘workshops’).insert({ name: workshopName });
...
async search(query) {
const { data } = await supabase.from(‘dtc_codes’)
.select(’*’)
...
const fullName = registerForm.querySelector(’#r-name’)?.value?.trim();
const email = registerForm.querySelector(’#r-email’)?.value?.trim();
const workshopName = registerForm.querySelector(’#r-shop’)?.value?.trim();
const password = registerForm.querySelector(’#r-pass’)?.value;
if (!fullName || !email || !password) { showError(‘Preencha todos os campos.’); return; }
if (password.length < 8) { showError(‘Senha deve ter pelo menos 8 caracteres.’); return; }
...
el.style.cssText = `position:fixed;bottom:24px;left:50%;transform:translateX(-50%); background:${type==='error'?'#7a0f0a':'#14532d'}; color:#fff;padding:12px 24px;border-radius:3px; font-family:sans-serif;font-size:14px;font-weight:500; z-index:9999;box-shadow:0 8px 32px rgba(0,0,0,.4);`;
return el;
}
...
if (msg.includes(‘Password should’)) return ‘Senha deve ter pelo menos 8 caracteres.’;
